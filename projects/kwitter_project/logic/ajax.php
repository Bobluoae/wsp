...
if ($_GET["ajax"] == "like_reply") {
	...
	// Updatera datat på "likes" eller "dislikes" i DB
	if ($payload->isDislike == true) {
		$query = $conn->prepare("UPDATE replies SET dislikes = dislikes + 1 WHERE r_id = ?");
	} else {
		$query = $conn->prepare("UPDATE replies SET likes = likes + 1 WHERE r_id = ?");
	}
	...
	//Skicka en respons till webbläsaren
	header("Content-Type: application/json");
	echo json_encode("Ok!" . $payload->replyId);
	exit();
}
